                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {features.map((feature, index) => (
                        <FeatureCard key={index} {...feature}>
                            {feature.active && (
                                <Link href={"/evaluate-resumes"} className="hover:cursor-pointer mt-4 inline-block bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 transition duration-300">
                                    Start Now
                                </Link>
                            )}
                            {feature.comingSoon && (
